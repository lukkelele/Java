...
public class Player {
  ...
  public void addCard(Card.Mutable c) {
    hand.add(c);
  }
  ...
  /**
   * Looks in the players hand and and checks if it contains the card with the value submitted in the parameter.
   *
   * @return true if the card is found,  else false
   */
  public boolean hasCard(Card.Value value) {
    for (Card c : getHand()) {
      if (c.getValue() == value) {            
        return true;                                   
      } 
